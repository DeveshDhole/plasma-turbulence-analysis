...
def fif_lowfilter(f,M,tol = 1e-18, preprocess='extend-periodic', \
    ...
    wshrink=0,npad_raisedcos = None, fill_nan = 'linear'):
    ...
    in_f = f
    if fill_nan: in_f = np.interp(np.arange(np.size(in_f)), np.flatnonzero(~np.isnan(in_f)), in_f[~np.isnan(in_f)])
    wsh = 0
...
def gaussian_filter1d(f,sigma = 1):
    from scipy.signal import fftconvolve
    from scipy.signal.windows import gaussian

    # Create a Gaussian kernel
    sigma = int(sigma)
    kernel_size = 5*int(sigma)
    gauss_kernel = gaussian(kernel_size, std=sigma)
    gauss_kernel /= np.sum(gauss_kernel)  # Normalize
    
    # Apply convolution
    smoothed = fftconvolve(f, gauss_kernel, mode='same')
